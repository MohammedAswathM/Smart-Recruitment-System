...
def ranking(request, id):
    ...
    jobfilename = f"{job_data.company_name}_{job_data.title}.txt"
    job_desc = f"{job_data.details}\n{job_data.responsibilities}\n{job_data.experience}\n"
    ...
    if not resumes_data.exists():
        messages.error(request, "No resumes available to rank for this job.")
        return redirect('job-listings')

    try:
        result_arr = screen.res(resumes_data, job_data)
    except IndexError as e:
        print("IndexError in screen.res:", e)
        messages.error(request, "An error occurred while ranking resumes.")
        result_arr = []
...
try:
    generator = pipeline("text-generation", model="gpt2")  # Replace with "distilgpt2" for lighter model if needed
except Exception as e:
    logging.error("Error initializing Hugging Face pipeline: %s", str(e))
    generator = None

def interview_page(request):
    """
    Render the interview page and handle form submissions.
    """
    # Fetch all ranked resumes without filtering by job title
    ranked_resumes = RankedResume.objects.all().order_by('-score')

    if request.method == "POST":
        # Get data from the POST request
        job_description = request.POST.get("job_description", "")
        resume_name = request.POST.get("resume_name", "")
        resume_score = request.POST.get("resume_score", "")

        # Validate input fields
        if not job_description or not resume_name or not resume_score:
            return JsonResponse({"error": "All fields are required"}, status=400)

        # Validate that the generator is initialized
        if not generator:
            return JsonResponse({"error": "The text generator is not available. Please check the server logs."}, status=500)

        # Generate interview questions
        questions = generate_questions_huggingface(job_description, resume_score)
        return JsonResponse({"questions": questions})

    # Render the HTML page with ranked resumes
    return render(request, "interview_page.html", {"ranked_resumes": ranked_resumes})


def generate_questions_huggingface(job_description, resume_score):
    """
    Generate interview questions based on the job description and resume score.
    """
    prompt = f"""
    You are an expert interviewer with a focus on technical skills and real-world experience. Based on the following job description:
    {job_description}

    And considering the candidate's resume score of {resume_score}, generate 5 unique, detailed, and job-specific interview questions. These questions should assess the candidate's technical expertise, relevant experience, and their fit for the role. Ensure the questions are clear, challenging, and directly tied to the job requirements and industry standards.
    """
    try:
        ...
        # Assume each question ends with a period, newline, or bullet point
